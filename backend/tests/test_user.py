...
class TestRegisterUser:
    @pytest.mark.anyio
    async def test_不正なメールアドレスはエラーになること(self):
        with pytest.raises(UserRegistrationError) as exc:
            await register_user("not-an-email", "ValidPass1!")
        assert exc.value.type == "INVALID_EMAIL_ADDRESS"

    @pytest.mark.anyio
    async def test_不正なパスワードはエラーになること(self):
        with pytest.raises(UserRegistrationError) as exc:
            await register_user("user@example.com", "short")
        assert exc.value.type == "INVALID_PASSWORD"

    @pytest.mark.anyio
    async def test_同じメールアドレスのユーザが既に登録されている場合はエラーになること(self):
        ...
        assert exc.value.type == "ALREADY_EXISTS"

    @pytest.mark.anyio
    async def test_ユーザ登録ができること(self):
...
class TestVerifyUser:
    @pytest.mark.anyio
    async def test_ユーザが存在しない場合にNoneを返すこと(self):
        mock_db = _make_mock_db(existing_user=None)
        with _patch_session(mock_db):
    ...
    @pytest.mark.anyio
    async def test_パスワードが誤っている場合にNoneを返すこと(self):
    ...
    @pytest.mark.anyio
    async def test_認証情報が正しい場合にユーザ情報が返されること(self):
